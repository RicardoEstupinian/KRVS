        ...
        if 'btnsimular' in request.POST:
            ...
            for dato in datoSimulacion:
                if int(dato.id) == int(idS):
                    velocidad_Cabina = request.POST.get('velocidad_Cabina')
                    ...
                    velocidad_P = float(velocidad_Cabina)/2
                    ...
                    for lamda in dato_Tipo:
                        ...
                        carga_Maxima = (
                            (3.1416*3.1416)*(210000)*lamda.momento_De_Inercia)/(2*(longitud_Embolo*longitud_Embolo))
                        if carga_Maxima >= carga_Piston	:
                            aceite_Circulacion = lamda.aceite * recorrido_Total_Piston
                            break
                    caudal_Bomba = float(velocidad_Cabina)*(((5*6*3.1416)*((float(diametro_Piston)/100)*(float(diametro_Piston)/100)))/4)
